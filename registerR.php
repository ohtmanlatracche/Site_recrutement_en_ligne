...
$sql = "SELECT COUNT(*) as count FROM recruteur WHERE email = :email";
...
// Préparer la requête pour vérifier si l'email est déjà utilisé
$sql1 = "SELECT COUNT(*) as countC FROM recruteur WHERE cin = :cin";
...
else 
{
     if( $pass!=$confirm){
      echo '<span style="color: red;font-weight: bold;font-size:22px;" >Les mots de passe ne sont pas identiques</span>';
         ...
         $insertionClient = $conn->prepare('INSERT INTO recruteur(cin,firstname,lastname,email,pass,confrm) VALUES(?,?,?,?,?,?)');
         $insertionClient->execute(array($cin,$firstname,$lastname,$email,$pass,$confirm));

    echo '<span style="color:green ;font-weight: bold;font-size:22px;margin-left:330px ;" > ✔️Votre compte a été crée avec succé</span>';
          }
}
}  
}
    else{
    echo '<span style="color:red ;font-weight: bold;font-size:22px;margin-left:330px ;" >❌Attention, Tous Les Champs Sont Obligatoires</span>';
  }
}
...
<div class="container2">
  ...
  <label for="cin">CIN</label>
    ...
    <br><br>
    <p>Vous avez déjà un compte ? <a class="button" href="AuthentificationR.php">Se
        connecter</a></p>
  </form>
